<template>
  <!-- 发布资讯页面 -->
  <div>
    <el-form ref="form" :model="form" :rules="rules" label-width="80px" align="left">
      ...
      <el-form-item label="封面" style="float: right; margin-right: 100px" prop="url">
        <el-upload
      ...
      <el-form-item label="正文" prop="description" style="clear: both">
        <mavon-editor
          ref="md"
      ...
      <el-form-item>
        <div style="text-align: right">
          ...
          <el-button type="success" @click="submit('form')">发布</el-button>
        </div>
      </el-form-item>
    </el-form>
  </div>
...
export default {
  ...
  methods: {
    ...
    submit(form) {
      this.$refs[form].validate(valid => {
        if (valid) {
          // 临时加上
          this.form.author = window.localStorage.getItem('name')
  ...
  created() {
    if (this.$route.params.id != -0) {
      this.detailsArticle(this.$route.params.id).then(() => {
        this.form = _.clone(this.details[0])
        this.imageUrl = process.env.VUE_APP_BASE_API + `/public/upload/` + this.form.url
      })
    }
  }
}
