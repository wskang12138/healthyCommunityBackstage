<template>
  <div class="user_details">
    ...
    <div style="text-align: right; padding-right: 220px; border-top: 1px solid seashell">
      ...
      <el-button size="small" type="text" @click="handleResetPwd">重置密码</el-button>
    </div>

    <!-- 个人信息模态框 -->
    <el-dialog title="个人信息修改" :visible.sync="visible" width="50%" class="customer_modal">
      <el-form ref="form" size="small" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="账号">
          <el-input disabled :placeholder="form.account" />
        </el-form-item>
    ...
    <el-dialog title="更改头像" :visible.sync="visible_userface" width="50%" class="customer_modal">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" size="small" @click="alterfaceHandle">确 定 修 改</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState('users', ['people'])
  },

  methods: {
    ...mapActions('users', ['peopleFind', 'passwordChange', 'changeImg', 'updatePeople']),
    ...
    alterfaceHandle() {
      ...
      this.changeImg(this.obj).then(res => {
        this.peopleFind({ token: Cookies.get('wsk-token') }).then(() => {
          this.imageUrl = process.env.VUE_APP_BASE_API + `/public/upload/` + this.people.avatar
        })
        window.localStorage.setItem('avatar', this.people.avatar)
        this.$message({
          type: 'success',
          message: res.message
        })
    ...
    alterUserfaceHandler(form) {
      this.$refs[form].validate(valid => {
        if (valid) {
          this.updatePeople(this.form).then(res => {
            this.peopleFind({ token: Cookies.get('wsk-token') }).then(() => {
              this.imageUrl = process.env.VUE_APP_BASE_API + `/public/upload/` + this.people.avatar
            })
            this.visible = false
