<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="确诊时间" align="center">
    ...
    <el-dialog :title="title" :visible.sync="dialogFormVisible">
      <el-form ref="proFrom" :model="adddisease" :rules="rules">
        ...
        <el-form-item label="疾病名称" :label-width="formLabelWidth" prop="diease_name">
          <el-input v-model="adddisease.diease_name" placeholder="请输入疾病名称" />
        </el-form-item>
        <el-form-item label="确诊时间" :label-width="formLabelWidth" prop="q_time">
          <el-date-picker
            v-model="adddisease.q_time"
            type="datetime"
...
export default {
  ...
  computed: {
    ...mapState('nowDieases', ['Diease', 'user'])
  },
  methods: {
    ...mapActions('nowDieases', ['findDiease', , 'deleteDiease', 'findUser', 'saveOrUpdateDiease']),
    //   点击添加按钮
    hanlderqueryDiease() {
      // 过滤查询参数，将空字符串的键值对删除掉
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.now_id)
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const id = row.now_id || Array.from(this.ids).join(',') // 数据类型得转化不然无法实现批量删除
      this.$confirm('是否确认删除数据项?', '警告', {
        ...
        .then(() => {
          return this.deleteDiease({ now_id: id })
        })
        .then(() => {
          this.hanlderqueryDiease()
          this.$message({
        ...
        if (valid) {
          ...
          for (const index in arr) {
            if (arr[index].username == this.adddisease.username) {
              this.adddisease.x_user_id = arr[index].user_id
            }
          }
    ...
    download(data) {
      const downloadUrl = `${process.env.VUE_APP_BASE_API}/nowDiease/download?data=${JSON.stringify(
        data
      )}`
