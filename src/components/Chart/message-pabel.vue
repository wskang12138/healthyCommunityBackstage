<template>
  <div class="message-pabel-box">
    <el-button
      ...
      @click="eyeMore"
    >加载更多消息</el-button>
    <ul v-if="nowSwitchId == this.id ? false : true" class="message-styles-box">
      <li v-for="(item, index) in messageTemplate()" :key="index" :class="judgeClass(item)">
        <img class="message-avatar" :src="urlHandle(item.avatar)">
        <p v-if="item.type == 'server'" class="message-nickname">
          {{ item.nickName }} {{ formatTime(item.message.time) }}
        </p>
        <p v-else class="message-nickname">
          {{ formatTime(item.message.time) }} {{ item.nickName }}
        </p>
        <p class="message-classic" v-html="item.message.content" />
      </li>
    </ul>
  </div>
</template>
  ...
  mounted() {
    ...
    this.sockets.subscribe('USER_MESSAGE', result => {
      ...
      for (let i = 0; i < result.length; i++) {
        let temp = JSON.parse(result[i].body)
        if (
