    ...
    <el-container>
      ...
      <el-main>
        <el-header height="40px">
          ...
          <span v-else class="title">{{ concats[nowSwitch].nickName }}</span>
        </el-header>
        ...
        <message-input :concats="concats" :local-info="localInfo" :now-switch-id="nowSwitchId" />
      </el-main>
      <footer class="footer" />
    </el-container>
  </div>
  ...
  mounted() {
      ...
      if (result.length > 0) {
        for (let i = 0; i < this.concats.length; i++) {
          if (this.concats[i].id == 'group') {
            ...
            this.concats[i].message.content = temp.message.content
            this.concats[i].message.time = temp.message.time
          }
        }
      }
    })
    ...
    this.sockets.subscribe('onLine', res => {
      ...
      if (code === 1) {
        this.concats = body
        for (let i = 0; i < this.concats.length; i++) {
          this.concats[i].isShowMore = true
          this.concats[i].page = 0
        }
        this.nowSwitchId = 'robots'
      }
    })
